...
contexts/AuthContext.tsx   app-wide auth state (user, register, login, logout, resetPassword)
screens/                   SignInScreen, RegisterScreen, HomeScreen, AddEditHabitScreen, CalendarScreen
components/                HabitCard, StreakBadge, EmptyState, Fab, Chip, M3TextField, PressableScale
hooks/                     useHabits, useCheckIns (data hooks screens call)
services/habitService.ts   all Firestore reads/writes (data layer)
firebase/firebaseConfig.ts Firebase init + auth functions
utils/                     date + streak calculation helpers
types/habit.ts             Habit, HabitInput, Frequency, StreakCounts
types/navigation.ts        typed param lists + screen props for every navigator
firestore.rules            security rules to paste into Firebase console
tsconfig.json              extends expo/tsconfig.base, strict mode on
```

This project is TypeScript throughout (`.ts`/`.tsx`). Only the tooling configs
(`babel.config.js`, `metro.config.js`, `tailwind.config.js`) stay as plain
`.js`, since that's what those tools expect.
